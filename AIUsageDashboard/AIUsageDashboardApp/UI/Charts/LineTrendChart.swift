...
/// Token-usage-over-time line (design spec §3). Accent line over the one
/// sanctioned pink→transparent area gradient, trailing dot + value label on the
/// most recent point, muted mono axes, horizontal gridlines only. Plain value
/// inputs — no view-model coupling. Honest empty state when history < 2 points.
        ...
        .chartXAxis {
            AxisMarks(values: .automatic(desiredCount: 4)) { _ in
                AxisValueLabel(format: .dateTime.month(.abbreviated).day(), centered: false)
                    .font(.mono(size: 10))
                    .foregroundStyle(PadzyTheme.muted)
            }
        }
        .chartYAxis {
            AxisMarks(position: .leading, values: .automatic(desiredCount: 3)) { value in
                AxisGridLine()
                    .foregroundStyle(PadzyTheme.muted.opacity(0.15))
                AxisValueLabel {
